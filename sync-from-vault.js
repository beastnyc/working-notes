...
const OBSIDIAN_VAULT_PATH = '/Users/bianca/documents/mind'; // Your Obsidian vault
const NOTES_FOLDER = '_notes'; // The folder to sync
...
function syncNotes() {
    try {
        const vaultNotesPath = path.join(OBSIDIAN_VAULT_PATH, NOTES_FOLDER);
        ...
        // Check if _notes folder exists in vault
        if (!fs.existsSync(vaultNotesPath)) {
            console.error(`❌ _notes folder not found in vault: ${vaultNotesPath}`);
            console.log('Please create a _notes folder in your Obsidian vault');
            process.exit(1);
        }
        ...
        // Copy only the _notes folder from vault
        copyDirectory(vaultNotesPath, WORKING_NOTES_PATH);
        ...
        rl.question('🚀 Push to GitHub? (y/n): ', (answer) => {
            if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
                rl.question('📝 Commit message: ', (commitMessage) => {
                    pushToGitHub(commitMessage || 'Update notes from Obsidian vault');
                    rl.close();
                });
            } else {
